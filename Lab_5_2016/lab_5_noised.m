...
if(shiftmax<0)|(shiftmax>=550000) 
    ...
    search_area=0.1; %область поиска относительно центра        
   for ki=1:2 %Цикл определяет количество итераций
     ...
     noise2=randn(kt);%генерация массива нормально распределенного шума 
     for i=1:kt  %генерация эталонного радиоимпульса                 
           x(i)=w1(i)*cos(2*pi*(i-(shift1))/75)+Q*noise2(i);                   
     end
%      i=1:kt;
%      figure
%      plot(i,x(i));
...
%вычисление СКО и коэффициента корреляции X и Y
        ...
        %        kor1(k)=kor1(k)+sxy(i);%вычисление нормы Минковского 
            ...
            kor1(k)=kor(k)/(sqrt(x_sko*y_sko));%вычисление коэффициента
           % корреляции X и Y
           kor(k)=kor1(k);
        end        
        xx=1:k;
